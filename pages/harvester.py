...
def main():
	...
	origin_pool = (config['Mandatory']['origin']).split(',')
	...
	def rdf(urlrdf, f, num):		
		...
		for s, p, o in input:
			g.add((s, p, o))
			#Triples to link the data with the country that it comes from
			g.add((s, URIRef('http://origin'), Literal(origin_pool[num])))
